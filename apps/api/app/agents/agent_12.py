...
from app.agents.prompts.agent_12 import (
    AGENT_12_SYSTEM_PROMPT_TEMPLATE,
    AGENT_12_FIX_SYSTEM_PROMPT_TEMPLATE,
)
from app.agents.artifacts import save_artifact
from app.models.agent_11 import GeneratedFile
from app.models.agent_12 import ModuleReviewOutput, Agent12Output
...
async def _review_module(state: PipelineState, module: dict) -> ModuleReviewOutput:
    system_prompt = AGENT_12_SYSTEM_PROMPT_TEMPLATE.format(
    ...
    return await run_agent(
        ...
        agent_name=f"agent_12_backend_gate::{module['module_name']}::cycle_{state.be_gate_cycle}",
...
async def agent_12_node(state: PipelineState) -> PipelineState:
    modules = state.agent_11_output["modules"]
    ...
    output = Agent12Output(
        overall_result=overall, module_reviews=list(module_reviews)
    )

    await _generate_agent_12_artifacts(state.run_id, output, state.be_gate_cycle)

    return state.model_copy(
        update={
            "agent_12_output": output.model_dump(),
            "current_agent": "agent_12_backend_gate",
            "status": "running",
        }
    )
...
async def _fix_file(
    ...
    system_prompt = AGENT_12_FIX_SYSTEM_PROMPT_TEMPLATE.format(
    ...
    result = await run_agent(
        run_id=state.run_id,
        organization_id=state.organization_id,
        agent_name=f"agent_11_fix::{module_name}::{file['path']}::cycle_{state.be_gate_cycle}",
        model="claude-sonnet-5",
        system_prompt=system_prompt,
        user_message=json.dumps(input_payload, indent=2),
...
async def agent_11_fix_node(state: PipelineState) -> PipelineState:
    """Applies targeted fixes to only the backend files that failed review."""
    review = state.agent_12_output
    modules = {m["module_name"]: m for m in state.agent_11_output["modules"]}
    ...
    updated_modules = json.loads(
        json.dumps(state.agent_11_output["modules"])
    )  # deep copy
    ...
    return state.model_copy(
        update={
            "agent_11_output": {"modules": updated_modules},
            "be_gate_cycle": state.be_gate_cycle + 1,
        }
    )


def route_after_backend_gate(state: PipelineState) -> str:
    if state.agent_12_output["overall_result"] == "PASS":
        return "agent_13_integration"
    if state.be_gate_cycle >= MAX_GATE_CYCLES:
        return "agent_13_integration"   # exhausted retries — proceed anyway, flagged
    return "agent_11_fix"


async def _generate_agent_12_artifacts(
    run_id: str, output: Agent12Output, cycle: int
):
    all_file_reviews = [
    ...
    await save_artifact(
        ...
        f"be_review_report_cycle_{cycle}.md",
        review_md,
    )
    await save_artifact(
        run_id,
        "agent_12_backend_gate",
        f"be_fix_tasks_cycle_{cycle}.json",
        json.dumps(
