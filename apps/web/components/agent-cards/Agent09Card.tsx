...
interface AgentOutput {
  agent_name: string;
  status: string;
  structured_output?: Agent09Output;
  model_used?: string;
  duration_ms?: number;
}

interface Agent09CardProps {
  agent: AgentOutput;
}
...
export function Agent09Card({ agent }: Agent09CardProps) {
        ...
        <div className="p-4 space-y-3">
          <div className="flex items-center gap-3 mb-3">
            ...
            <span className="text-sm font-medium text-gray-700">
              Reviewing frontend code across modules...
            </span>
          </div>
          <div className="grid grid-cols-2 gap-2">
            {["Auth Module", "Dashboard Module", "Admin Panel", "Shared Components"].map(
              (name) => (
  ...
  return (
    <div className="p-4 space-y-4 text-sm">
          ...
          <div>
            ...
            <div className="space-y-2">
              {activeModule.fix_tasks.map((task) => (
                <div
                  key={task.id}
                  className={`p-3 rounded-lg border ${getSeverityColor(task.severity)}`}
                >
      ...
      {/* Exhausted retries banner — FAIL + completed means retries were exhausted */}
      {out?.overall_result === "FAIL" && agent.status === "completed" && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
          <p className="text-xs text-yellow-800">
            ⚠️ Frontend review didn&apos;t fully pass after {MAX_GATE_CYCLES} fix cycles — proceeding
            to backend with known issues flagged. Review{" "}
            <span className="font-mono">fe_review_report_cycle_{MAX_GATE_CYCLES}.md</span> manually
            before deploying.
          </p>
        </div>
      )}
